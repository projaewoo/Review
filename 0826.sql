...
--사원테이블에서 사번, 사원이름, 직무, 월급, 보너스, 연봉을 조회하시오.
--단, 연봉은 내림차순으로 출력하시오.
SELECT empno AS "사번", ename AS "사원이름",
            job AS "직무", sal AS "월급",
            comm AS "보너스",
...
--5. NULL처리
--NVL() 함수
--SELECT NVL(comm, 'NULL')
--comm : 숫자형태이므로 숫자형으로 변환
SELECT NVL(mgr, 0), job
FROM emp;
...
--사원 중에 월급이 1000불 이하인 사원의 정보를 출력.
SELECT  *
FROM    emp
WHERE  deptno = 20 AND sal <= 1000;

--사원테이블에서 부서번호가 10번이거나 월급이 3000불에서 5000불 사이인 사람의
--사번, 이름, 월급, 부서번호를 조회하시오.
SELECT  empno, ename, sal, deptno
FROM    emp
WHERE deptno = 10 OR (sal >= 3000 AND sal <= 5000);

--부서테이블에서 부서번호가 10, 20이 아닌 사원의
--부서번호, 부서이름, 위치를 조회하시오.
